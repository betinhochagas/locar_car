...
export async function getSiteSettings(category?: string): Promise<SiteConfig[]> {
  try {
    const url = category 
      ? `${API_BASE_URL}/site-settings.php?category=${encodeURIComponent(category)}`
...
export async function getSiteSetting(key: string): Promise<SiteConfig> {
  try {
    const response = await fetch(`${API_BASE_URL}/site-settings.php?key=${encodeURIComponent(key)}`);
...
export async function toggleSectionActive(id: number): Promise<PageSection> {
  try {
    ...
    console.error('Erro ao alternar status da seção:', error);
    throw error;
  }
}
